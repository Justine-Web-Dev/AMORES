...
function ForceChangePasswordModal({ isOpen, onSuccess, currentPasswordProp, token }) {
  ...
  const [currentPassword, setCurrentPassword] = useState(currentPasswordProp || '');
      ...
      <div className="bg-white p-8 rounded-2xl shadow-xl border border-slate-200 max-w-md w-full mx-4 relative">
        ...
        <div className="bg-amber-50 border-l-4 border-amber-500 p-4 rounded-r-md mb-6">
          ...
          <p className="text-sm text-amber-600 mt-1">
            You are logging in with a system-generated password. Please change it now to secure your account.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="pt-4 flex justify-end">
            <button
              type="submit"
              disabled={isLoading}
              className={`w-full py-3 rounded-xl text-white font-medium transition-all duration-200 shadow-md flex items-center justify-center gap-2 ${
                isLoading 
