...
import { RiDashboardLine, RiFileTextLine } from "react-icons/ri";
import { FiLogOut, FiChevronLeft, FiChevronRight } from "react-icons/fi";

function SidebarInterviewer({ isCollapsed, setIsCollapsed }) {
      ...
      <div className={`${isCollapsed ? 'w-[90px]' : 'w-64'} transition-[width] duration-300 ease-in-out h-screen bg-gradient-to-b from-[#F8FAFC] to-[#EFF2F6] border-r border-slate-200 flex flex-col fixed left-0 top-0 z-40 select-none`}>
        ...
        <nav className={`flex-1 p-4 space-y-1 scrollbar-thin ${isCollapsed ? 'overflow-visible' : 'overflow-y-auto'}`}>
          <ul className="space-y-1">
            <li>
              <Link className={linkClass('/InterviewDashboard')} to="/InterviewDashboard">
                <RiDashboardLine size={18} className={isActive('/InterviewDashboard') ? 'text-white' : 'text-slate-400 group-hover:text-slate-600'} />
            ...
            <li>
              <Link className={linkClass('/InterviewDashboard/applications')} to="/InterviewDashboard/applications">
                <RiFileTextLine size={18} className={isActive('/InterviewDashboard/applications') ? 'text-white' : 'text-slate-400 group-hover:text-slate-600'} />
...
export default SidebarInterviewer
